import React from "react";

export default function Hightlight() {
  return (
    <section className="bg-black-secondary rounded-2xl">
      <img
	  className="rounded-t-2xl lg:h-[221px] xl:h-[401px] w-full object-cover object-center"
        src="https://s3-alpha-sig.figma.com/img/8ac9/e0e3/bcf2d5507a81254146143702cd71f923?Expires=1698624000&Signature=AKFrlLZ70kmMLKXck4arBl1bEucOmhnuI6qi6nqmuttuXj7dZcnwBMmwz-I5bB4tH~hcU268f7P6JLt-x216ceKUB2rRH2eBMWqrHLqDOgUBiaN-qJ9ewEpfCKXwG--8CFfRXAkDoZ4N11qxDonnCaMYGJd4icE1pyrpmHWAtKP6YCxs6n~wGFKb3Iv4Oy36OK--b9HxOdSwF17DtogvSZORAWgl1jh2yfI7Gr08Oj57VC8BQr178Dk1NTXDJ7qEHDsOBcNGAjmVSV8PjQHwqRytX8NluOdFqgvkHfMuCUmPfcl7AA3BPcWdq9Sxw1hHTaaU3t8NsTi0F~PcMXuqUA__&Key-Pair-Id=APKAQ4GOSFWCVNEHN3O4"
        alt=""
      />
      <div className="p-5 grid gap-3 ">
        <h1>Space Walking</h1>
        <div className="flex items-center gap-3">
          <img
		  	className="w-6 rounded-full"
            src="https://s3-alpha-sig.figma.com/img/9acf/2677/568b38bc98ba36dbd43c768c40de6716?Expires=1698624000&Signature=B9QpGxaojZUWtHk5pfj31hEUmOEM3awGPCAE64pa-7tfBkVpVTKl2~xb~ODNTNRpNBetv1uHPobYlEP6~G9IrsbqDTHXQB8N~DGhSiN8IeiAjU-IfWKpvuYVH9vG4QC2EfSteEo6F8QEa0MNGHuOQuWtc0qIDT8Buk8r74~3IWR~cuJxknRSU2HGiJmaYRQ4PYApL5BkRwmHTfT7CtIcl2i0oPin~8gvNrCC29NLRIqSYlVb8poUOHl3mf0Ht4cj4AdQa3WjR0eU2Sl4YUshWs5eaAzpIlog9Y~RP9LtdPDLXoyoWfgb9-da9zrSJRaerA326LZYlZ3nLcPJ28NT4A__&Key-Pair-Id=APKAQ4GOSFWCVNEHN3O4"
            alt=""
          />
          <caption>Animakid</caption>
        </div>
      </div>
    </section>
  );
}
